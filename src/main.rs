...
fn type_eq(a: &Option<TileType>, b: &Option<TileType>) -> bool {
    matches!(
        (a, b),
        (&Option::Some(..), &Option::Some(..)) | (&Option::None, &Option::None)
    )
}
...
fn main() {
    ...
    const TILES_H: usize = 8;
    const TILES_W: usize = 8;
    ...
    let mut first_y_tiles: Vec<Tile> = Vec::new();
    let mut first_x_tiles: Vec<Tile> = Vec::new();
    for y in 0..TILES_H {
        first_y_tiles.push(Tile::new(next_tile));
        ...
        for x in 0..TILES_W {
            ...
            for y1 in 0..(HEIGHT / TILES_H) {
                for x1 in 0..(WIDTH / TILES_W) {
                    ...
                    pixels[((y * HEIGHT / TILES_H) + y1) * WIDTH + ((x * WIDTH / TILES_W) + x1)] =
            ...
            // get the next tile
            if y > 0 && x != (TILES_W - 1) {
                let vs1: Vec<u8> = last_x_tiles[x + 1].get_possible(BOTTOM);
                let vs2: Vec<u8> = tile.get_possible(LEFT);
                vs = vs2
                    .iter()
                ...
                vs = if y > 2 && y == (TILES_H - 2) {
                    let vs1 = x_buffer[0].get_possible(BOTTOM);
                    let vs2 = first_x_tiles[0].get_possible(TOP);
                    vs2
                        .iter()
                        .filter(|v| vs1.contains(*v))
                        .copied()
                    ...
                    x_buffer[0].get_possible(BOTTOM)
                };
            } else {
                vs = tile.get_possible(LEFT);
            }
            if x > 2 && x == (TILES_W - 2) {
                let vs2 = first_y_tiles[y].get_possible(RIGHT);
                vs = vs2
                    .iter()
            ...
            if y == (TILES_H - 1) && x != (TILES_W - 1) {
                let vs2 = first_x_tiles[x + 1].get_possible(TOP);
                vs = vs2
                    .iter()
                    .filter(|v| vs.contains(*v))
                    .copied()
